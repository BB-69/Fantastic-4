...
public class AnimatedMenuBackground extends Node {
    ...
    private float birdBaseX = GameCanvas.WIDTH / 2f;
    private float birdBaseY = GameCanvas.HEIGHT / 2f;
    ...
    public void update() {
        ...
        sea1.x -= seaSpeed; 
        sea2.x -= seaSpeed;
        if (sea1.x <= -GameCanvas.WIDTH / 2f) sea1.x = sea2.x + GameCanvas.WIDTH;
        if (sea2.x <= -GameCanvas.WIDTH / 2f) sea2.x = sea1.x + GameCanvas.WIDTH;
        //เลื่อนฉากหน้า (ถนน/หาดทราย)
        float offsetSpeed = 200f * dt;
        offsetImage1.x -= offsetSpeed; 
        offsetImage2.x -= offsetSpeed;
        if (offsetImage1.x <= -GameCanvas.WIDTH / 2f) offsetImage1.x = offsetImage2.x + GameCanvas.WIDTH;
        if (offsetImage2.x <= -GameCanvas.WIDTH / 2f) offsetImage2.x = offsetImage1.x + GameCanvas.WIDTH;
        //ทำให้รถสั่น (ขยับเฉพาะแกน Y ขึ้นลงตามเวลา)
        car.y = carBaseY + (float)(Math.sin(timer * 20) * 2); 
        //ทำให้นกบินอยู่ที่เดิม (ขยับขึ้นลงช้ากว่ารถ เพื่อให้ดูเหมือนกำลังกระพือปีกร่อนอยู่)
        bird.y = birdBaseY + (float)(Math.sin(timer * 10) * 5); 
    }
    @Override
